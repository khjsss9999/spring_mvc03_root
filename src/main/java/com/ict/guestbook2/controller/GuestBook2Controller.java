...
public class GuestBook2Controller {
	...
	// 암호는 스프링 security에서 지원하므로 pom.xml에서 추가해야 한다.
	// spring-security.xml를 만들어서 bean 생성
	// web.xml에서 지정해줘야 spring-security.mxl 읽을 수 있도록 지정
	@Autowired
	...
	public ModelAndView getGuestBook2Insert() {
		return new ModelAndView("guestbook2/write");
	}

	@PostMapping("gb2_write_ok.do")
	public ModelAndView getGuestBook2InsertOK(GuestBook2VO g2vo, HttpServletRequest request) {
		try {
	...
	public ModelAndView getGuestBook2DeleteOK(GuestBook2VO g2vo) {
		...
		// passwordEncoder.matches(암호화 되지 않은 암호, 암호화 된 암호)
		// 일치하면 true, 아니면 false
